...
- **No CookieYes.** The cookie-disclosure *table* on `/legal/cookies` is a static,
  hand-maintained table (`CookieDisclosureTable.tsx`) — keep it in sync by hand.
...
| `src/components/legal/CookieDisclosureTable.tsx` | Static, hand-maintained cookie table on `/legal/cookies` |
...
## 5. The cookie-disclosure table

`/legal/cookies` shows a full list of the cookies/storage the site uses. This is a
**static, hand-maintained table** in `src/components/legal/CookieDisclosureTable.tsx`
(previously auto-generated by CookieYes — removed 2026-07-03).

- **When you add or remove a tracker, update the `GROUPS` array in that file.**
  Group it under the matching consent category (essential / measurement / marketing).
- No env var, no third-party script, no CookieYes cookie — it's just JSX.
- The consent banner + gating (sections 1–4 above) is entirely separate and was
  never CookieYes.
