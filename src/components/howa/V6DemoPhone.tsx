    ...
    <section id="demo" className="bg-[#f4f1e9] pt-2 pb-16 lg:pb-20 scroll-mt-20">
      <div className="max-w-[1240px] mx-auto px-6 sm:px-10">
        <div className="text-center max-w-[600px] mx-auto mb-9">
          <p className="smallcaps text-[14px] tracking-[0.18em] text-[color:var(--color-gold-deep)] mb-3">Try it yourself</p>
        ...
        <div className="mx-auto w-[348px] max-w-full">
          <div className="relative rounded-[48px] bg-gradient-to-b from-[#26262a] via-[#141416] to-[#08080a] p-[14px] shadow-[0_48px_96px_-32px_rgba(30,22,8,0.62)] ring-1 ring-black/40">
            <div className="relative h-[680px] overflow-hidden rounded-[38px] bg-[#f5efe2] text-[#1d2a40] ring-1 ring-black/20">
                ...
                <div className="flex h-full flex-col">
                  ...
                  {saveCta && (
                    <a
                      ...
                      className="mx-3 mb-2 flex items-center justify-center gap-1.5 rounded-xl bg-[#1f3a2b] py-2.5 text-[14px] font-medium text-white shadow-[0_8px_20px_-10px_rgba(31,58,43,0.6)] transition-colors hover:bg-[#15291e]"
                    >
              ...
              {/* score-rise toast */}
              {bump > 0 && (
                <div className="pointer-events-none absolute bottom-[80px] left-1/2 z-40 -translate-x-1/2 rounded-full bg-[#1d2a40] px-3.5 py-1.5 text-[14px] text-[#f3ede0] shadow-lg">
                  Home health +{bump}
                </div>
              )}
          ...
          <p className="mt-4 text-center font-mono text-[14px] tracking-[0.02em] text-[color:var(--color-ink-soft)]/65">A live preview of the HoWA record. No account needed.</p>
        </div>
      </div>
    </section>
  );
  ...
  return (
    <div className="flex h-full flex-col px-6 pb-6 pt-12 text-center">
      <div className="flex flex-1 flex-col">
        ...
        <p className="mx-auto mt-2 max-w-[230px] text-[14px] leading-[1.5] text-[color:var(--color-ink-soft)]">
          HoWA links everything to your address, not your phone. Pop in a postcode to begin.
        </p>

        <div className="mt-6 text-left">
          <label className="text-[14px] uppercase tracking-[0.14em] text-[color:var(--color-ink-soft)]/70">Postcode</label>
          <input
            value={postcode}
          ...
          />
          <label className="mt-4 block text-[14px] uppercase tracking-[0.14em] text-[color:var(--color-ink-soft)]/70">House type</label>
          <div className="mt-2 flex flex-wrap gap-2">
            {HOUSE_TYPES.map((t) => (
              <button key={t} onClick={() => setHtype(t)} className={"rounded-full border px-3 py-1.5 text-[14px] transition-colors " + (htype === t ? "border-[#5f6a49] bg-[#5f6a49] text-white" : "border-[color:var(--color-gold)]/30 bg-white/55 text-[color:var(--color-ink)] hover:bg-white")}>{t}</button>
            ))}
          </div>
        </div>
      </div>
      ...
      <button onClick={onDone} className="mt-2 text-[14px] text-[color:var(--color-ink-soft)]/70 hover:text-[color:var(--color-ink)]">Skip for now</button>
    </div>
  );
}
  ...
  return (
    <div>
      <div className="flex items-start justify-between">
        <div>
          <p className="text-[14px] leading-none text-[color:var(--color-ink-soft)]/65">{addr}</p>
      ...
      <p className="mb-2 mt-5 text-[14px] font-medium text-[color:var(--color-ink-soft)]/85">Next actions</p>
      ...
      <p className="mb-2 mt-5 text-[14px] font-medium text-[color:var(--color-ink-soft)]/85">Upcoming</p>
...
function ScanScreen({ scanning, results, runScan }: { scanning: string | null; results: { label: string; lines: string[] }[]; runScan: (s: (typeof SCANS)[number]) => void }) {
  return (
    <div>
      ...
      <p className="mt-1 text-[14px] text-[color:var(--color-ink-soft)]/75">Choose a photo to scan. HoWA finds what needs attention and adds it to your record.</p>
        ...
        {SCANS.map((s) => {
          ...
          return (
            <button key={s.id} onClick={() => runScan(s)} disabled={!!scanning} className="group relative aspect-[4/3] overflow-hidden rounded-xl border border-[color:var(--color-gold)]/20 text-left disabled:opacity-80">
              ...
              <span className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/60 to-transparent px-2.5 py-1.5 text-[14px] font-medium text-white">{s.label}</span>
              {busy && (
                <span className="absolute inset-0 flex items-center justify-center bg-black/25 backdrop-blur-[1px]">
                  ...
                  <span className="text-[14px] font-medium text-white">Scanning…</span>
                </span>
              )}
            </button>
          );
      ...
      {results.length > 0 && (
        <>
          <p className="mb-2 mt-5 text-[14px] font-medium text-[color:var(--color-ink-soft)]/85">Found</p>
          <div className="space-y-2 pb-1">
            {results.map((r, i) => (
              <div key={i} className="rounded-xl border border-[color:var(--color-gold)]/20 bg-white/75 p-3">
                <p className="text-[14px] font-medium text-[#1d2a40]">{r.label}</p>
                <ul className="mt-1 space-y-1">
                  {r.lines.map((l) => (<li key={l} className="flex items-center gap-2 text-[14px] text-[color:var(--color-ink-soft)]"><Tick /> {l}</li>))}
                </ul>
              </div>
            ))}
          </div>
...
function DesignScreen({ subject, style, step, count, showBefore, setShowBefore, pickSubject, pickStyle, reset }: { subject: Subject | null; style: { key: string; label: string } | null; step: "subjects" | "choose" | "styling" | "after"; count: number; showBefore: boolean; setShowBefore: (v: boolean) => void; pickSubject: (s: Subject) => void; pickStyle: (st: { key: string; label: string }) => void; reset: () => void }) {
  if (step === "subjects" || !subject) {
    ...
    return (
      <div>
        ...
        <p className="mt-1 text-[14px] text-[color:var(--color-ink-soft)]/75">Pick a photo, choose a look, and see it transformed.</p>
        <p className="mt-1.5 text-[14px] font-medium text-[color:var(--color-gold-deep)]">{3 - count} free restyle{3 - count === 1 ? "" : "s"} left</p>
        <div className="mt-3 grid grid-cols-2 gap-2.5 pb-1">
          {SUBJECTS.map((s) => (
            <button key={s.id} onClick={() => pickSubject(s)} className="group relative aspect-[4/3] overflow-hidden rounded-xl border border-[color:var(--color-gold)]/20 text-left">
              ...
              <span className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/55 to-transparent px-2.5 py-1.5 text-[14px] font-medium text-white">{s.label}</span>
            </button>
          ))}
        </div>
      </div>
  ...
  return (
    <div>
      <button onClick={reset} className="mb-2 inline-flex items-center gap-1 text-[14px] text-[color:var(--color-ink-soft)] hover:text-[color:var(--color-ink)]">‹ Photos</button>
      ...
      <div className="relative mt-3 aspect-[16/10] overflow-hidden rounded-xl border border-[color:var(--color-gold)]/20 bg-black">
        ...
        {step === "styling" && (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/45 text-white">
            ...
            <Spinner />
            <span className="mt-2 text-[14px]">Restyling in {style?.label}…</span>
          </div>
        )}
        {step === "after" && (
          <button onClick={() => setShowBefore(!showBefore)} className="absolute right-2 top-2 rounded-full bg-black/55 px-3 py-1 text-[14px] text-white backdrop-blur-sm">
      ...
      {step !== "after" ? (
        <>
          <p className="mb-2 mt-4 text-[14px] font-medium text-[color:var(--color-ink-soft)]/85">Choose a style</p>
          <div className="space-y-2 pb-1">
            {styles.map((st) => (
        ...
        <div className="mt-4 space-y-2 pb-1">
          <p className="text-[14px] text-[color:var(--color-ink-soft)]">Saved to your home record. Estimated next steps added to tasks.</p>
...
function DesignLocked() {
  return (
    <div className="flex h-full flex-col items-center justify-center px-6 text-center">
      ...
      <p className="mt-3 text-[14px] text-[color:var(--color-ink-soft)]/60">Free to join · no card needed</p>
    </div>
  );
}
...
function DocsScreen({ docs, addable, addDoc }: { docs: { name: string; meta: string }[]; addable: { name: string; meta: string }[]; addDoc: (d: { name: string; meta: string }) => void }) {
  return (
    <div>
      ...
      <p className="mt-1 text-[14px] text-[color:var(--color-ink-soft)]/75">Pick a document to add to your record. Each one raises your home-health score.</p>

      {addable.length > 0 ? (
        <>
          <p className="mb-2 mt-4 text-[14px] font-medium text-[color:var(--color-ink-soft)]/85">Add to your record</p>
            ...
            {addable.map((d) => (
              <button key={d.name} onClick={() => addDoc(d)} className="flex w-full items-center gap-3 rounded-xl border border-dashed border-[color:var(--color-gold)]/45 bg-white/40 p-3 text-left transition-colors hover:bg-white/75">
                ...
                <div className="min-w-0 flex-1">
                  <p className="truncate text-[14px] text-[#1d2a40]">{d.name}</p>
                  <p className="text-[14px] text-[color:var(--color-ink-soft)]/65">{d.meta} · +3 score</p>
                </div>
                <span className="shrink-0 rounded-full bg-[#5f6a49] px-3 py-1 text-[14px] text-white">Add</span>
              </button>
            ))}
          </div>
        </>
      ) : (
        <p className="mt-4 rounded-xl border border-[color:var(--color-gold)]/20 bg-white/55 px-4 py-3 text-[14px] text-[color:var(--color-ink-soft)]">Your record is complete. Every document is in.</p>
      )}

      <p className="mb-2 mt-5 text-[14px] font-medium text-[color:var(--color-ink-soft)]/85">Your documents</p>
      <div className="space-y-2 pb-1">
        {docs.map((d, i) => (
          <div key={d.name + i} className="flex items-center gap-3 rounded-xl border border-[color:var(--color-gold)]/20 bg-white/75 p-3">
            ...
            <div className="min-w-0 flex-1">
              <p className="truncate text-[14px] text-[#1d2a40]">{d.name}</p>
              <p className="text-[14px] text-[color:var(--color-ink-soft)]/65">{d.meta}</p>
            </div>
...
function HealthScreen({ score }: { score: number }) {
  return (
    <div>
      ...
      <p className="mt-1 text-[14px] text-[color:var(--color-ink-soft)]/75">A single score for how your home is doing. It rises as you add documents and book care.</p>
      <div className="mt-5 flex flex-col items-center">
        <Ring value={score} size={132} stroke={9} big />
        <p className="mt-2 text-[14px] text-[color:var(--color-ink-soft)]">Good shape · last checked today</p>
      </div>
      <div className="mt-6 space-y-3.5 pb-1">
        {HEALTH.map((h) => (
          <div key={h.label}>
            <div className="mb-1 flex items-center justify-between text-[14px]"><span className="text-[#1d2a40]">{h.label}</span><span className="text-[color:var(--color-ink-soft)]">{h.v}%</span></div>
  ...
  return (
    <div className="absolute inset-0 z-40 flex flex-col bg-[#f5efe2]">
      <div className="flex items-center justify-between px-5 pt-12 pb-3">
        ...
        <p className="text-[14px] uppercase tracking-[0.16em] text-[color:var(--color-gold-deep)]">{booking.service}</p>
        <span className="w-12" />
      </div>
      <div className="flex-1 overflow-y-auto px-5 pb-6 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        {booking.step === "pros" && (
          <>
            ...
            <p className="mt-1 mb-4 text-[14px] text-[color:var(--color-ink-soft)]/75">Vetted local trades, near {`SW1A`}.</p>
            <div className="space-y-2.5">
              {PROS.map((p) => (
                <button key={p.name} onClick={() => pickPro(p.name)} className="flex w-full items-center justify-between rounded-xl border border-[color:var(--color-gold)]/20 bg-white/75 p-3.5 text-left transition-colors hover:bg-white">
                  <div>
                    ...
                    <p className="text-[14px] text-[color:var(--color-ink-soft)]/70">{p.trade} · {p.dist}</p>
                  </div>
                  <span className="flex items-center gap-1 text-[14px] text-[#5f6a49]"><Star /> {p.rating}</span>
                </button>
              ))}
            </div>
          </>
        ...
        {booking.step === "slot" && (
          <>
            ...
            <p className="mt-1 mb-4 text-[14px] text-[color:var(--color-ink-soft)]/75">{booking.pro} · earliest availability.</p>
            <div className="space-y-2.5">
              {SLOTS.map((s) => (
  ...
  return (
    <div className="relative shrink-0" style={{ width: size, height: size }}>
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        ...
        {big && <span className="text-[14px] text-[color:var(--color-ink-soft)]">home health</span>}
      </div>
    </div>
  );
}
...
function ActionRow({ label, cta, done, doneText, onClick }: { label: string; cta: string; done: boolean; doneText: string; onClick: () => void }) {
  return (
    <div className="flex items-center justify-between rounded-xl border border-[color:var(--color-gold)]/20 bg-white/80 px-3.5 py-2.5">
      ...
      {done ? (
        <span className="flex items-center gap-1 text-[14px] text-[#5f6a49]"><Tick /> {doneText}</span>
      ) : (
        <button onClick={onClick} className="rounded-lg bg-[#5f6a49] px-3 py-1.5 text-[14px] text-white transition-colors hover:bg-[#515b3e]">{cta}</button>
      )}
    </div>
  );
}
function UpRow({ label, meta }: { label: string; meta: string }) {
  return (
    <div className="flex items-center justify-between">
      <span className="flex items-center gap-2 text-[14px] text-[#1d2a40]"><span className="h-1.5 w-1.5 rounded-full bg-[color:var(--color-gold)]" />{label}</span>
      <span className="text-[14px] text-[color:var(--color-ink-soft)]/70">{meta}</span>
    </div>
  );
}
