...
/**
 * ScanReveal, a faithful generalisation of the HoWA design "scan-to-design"
 * reveal. Same grammar: a ~2s blueprint scan, then the photo resolves to colour
 * while the RIGHT-HAND OUTPUT SHEET fills in beat by beat (fields, an optional
 * number that counts up, and a written brief), ending in a disclaimer + Replay.
 *
 * Driven by a per-tool dataset so any scan-type tool (Repair, Garden, Property,
 * Documents, Plan) shows the fields it actually returns. Every field maps to a
 * real deliverable, so it stays truthful. Self-contained: CSS + a little React.
 *
 * Beat structure (mirrors the demo):
...
export type ScanRevealData = {
  ...
  imageAlt: string;
  imageLabel: string;
  /** §06 safety: shown beside the result. */
  source?: string;
  confidence?: string;
  scanCaption: string;
...
export function ScanReveal({ data, fields }: { data: ScanRevealData; fields: ScanField[] }) {
  ...
  // Beat timings (ms), matching the demo's cadence.
