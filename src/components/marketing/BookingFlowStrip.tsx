  ...
  return (
    <section className="px-[5vw] py-[clamp(44px,5.5vw,80px)] border-t border-house-brown/8" style={{ background: "var(--color-house-white)" }}>
      <div className="mx-auto max-w-[1100px] text-center">
        ...
        <h2 className="font-display text-[clamp(29px,3vw,43px)] leading-[1.08] text-house-brown mb-4">
          Booked, delivered, remembered.
        </h2>
        <p className="font-sans text-[16.5px] leading-[1.7] text-house-stone max-w-[60ch] mx-auto mb-10">
        ...
        <ol className="grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-5 gap-x-5 gap-y-8 text-left">
          {STEPS.map((step) => (
            <li key={step.n} className="flex flex-col">
              <span className="font-display text-[31px] leading-none text-house-gold-ink mb-3">{step.n}</span>
              <span aria-hidden className="block h-px w-8 bg-house-gold/40 mb-3" />
              <span className="font-sans text-[18px] leading-[1.5] text-house-brown">{step.label}</span>
            </li>
          ))}
        </ol>
        ...
        <div className="mt-11">
          <a
            ...
            className="inline-flex items-center justify-center font-sans text-[14px] tracking-[0.18em] uppercase text-house-brown bg-house-gold-ink border border-house-gold-dark px-8 py-4 no-underline transition-colors hover:brightness-110"
          >
            Book a service
          </a>
        </div>
