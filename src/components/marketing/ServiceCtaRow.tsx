 ...
 *
 * `mode` mirrors the service-page pricing UX (doc §10): survey/quote services
 * ask for a survey rather than an instant booking.
 */
const PHONE_DISPLAY = "0800 047 8738";
const PHONE_HREF = "tel:08000478738";

export function ServiceCtaRow({
  service,
  mode = "book",
  bookHref,
}: {
  ...
  mode?: "book" | "quote";
  /**
   * ServiceOS deep-link (from buildBookingUrl) that preselects this service.
   * When set, the primary CTA becomes a full-page navigation so the OBF opens
   * with the service already chosen. When omitted, it falls back to the in-place
   * `#open-booking-form` trigger (a fresh booking on the current page).
   */
  bookHref?: string;
}) {
