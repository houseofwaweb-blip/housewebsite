...
function ImagePlaceholder({
  ...
  return (
    <div className={`${ratio} flex w-full flex-col items-center justify-center border-2 border-dashed border-house-brown/30 bg-house-white/60 p-6 text-center`}>
      <p className="font-sans text-[13px] tracking-[0.22em] uppercase text-[color:var(--ins-ink)]">Image placeholder</p>
      <p className="mt-3 max-w-[42ch] font-sans text-[17px] leading-[1.55] text-house-brown/80">{spec.description}</p>
      <p className="mt-4 font-sans text-[14.5px] leading-[1.5] text-house-brown/55">
        {spec.dimensions}
        <br />
    ...
    <div className="bg-house-cream text-house-brown">
      ...
      <section className="px-[5vw] pt-20 pb-12">
        <div className="mx-auto grid max-w-[1120px] items-center gap-10 lg:grid-cols-2 lg:gap-16">
          <div>
            ...
            <h1 className="mt-4 font-display text-[clamp(35px,5vw,63px)] leading-[1.04] text-house-black">
              {data.hero.heading}
            </h1>
            ...
            <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:items-center">
              ...
              <a
                ...
                className="inline-flex items-center justify-center whitespace-nowrap border border-house-brown/30 px-7 py-3.5 font-sans text-[14px] tracking-[0.16em] uppercase text-house-brown no-underline transition-colors hover:border-[color:var(--ins-ink)]"
              >
                Call {SUPPORT_PHONE}
              </a>
            </div>
      ...
      <section className="px-[5vw] py-14">
        <div className="mx-auto grid max-w-[1120px] items-center gap-10 lg:grid-cols-[1.05fr_0.95fr] lg:gap-16">
          <div className="max-w-[58ch]">
            <h2 className="font-display text-[clamp(27px,3vw,39px)] leading-[1.12] text-house-black">
              {data.whyDifferent.heading}
            </h2>
            {data.whyDifferent.body.map((para, i) => (
              <p key={i} className="mt-5 font-sans text-[20px] leading-[1.7] text-house-brown/85">{para}</p>
            ))}
          </div>
          {data.whyImage ? (
          ...
          <div className="mx-auto grid max-w-[1120px] items-center gap-8 lg:grid-cols-[1fr_auto] lg:gap-12">
            <div>
              <div className="grid gap-6 sm:grid-cols-3">
                {data.evidence.map((e) => (
                  <div key={e.label}>
                    <p className="font-display text-[clamp(31px,3.4vw,47px)] leading-none text-[color:var(--ins-ink)]">{e.stat}</p>
                    <p className="mt-2 font-sans text-[17px] leading-[1.5] text-house-brown/75">{e.label}</p>
                  </div>
                ))}
              </div>
      ...
      <section className="px-[5vw] py-14" style={{ background: "var(--color-house-white)" }}>
        <div className="mx-auto max-w-[1120px]">
          <h2 className="mb-8 max-w-[24ch] font-display text-[clamp(25px,2.8vw,37px)] leading-[1.12] text-house-black">
            {data.detail.title}
          </h2>
        ...
        return (
          <section className="px-[5vw] py-14" style={{ background: "var(--color-house-cream-dark)" }}>
            <div className="mx-auto max-w-[1120px] border-l-4 border-[color:var(--ins-accent)] bg-house-white/70 p-7 sm:p-9">
              <p className="font-sans text-[14px] tracking-[0.24em] uppercase text-[color:var(--ins-ink)]">
                Read this before you rely on cover
              </p>
              <h2 className="mt-3 max-w-[28ch] font-display text-[clamp(27px,3vw,39px)] leading-[1.12] text-house-black">
              ...
              {lim.note ? (
                <p className="mt-7 font-sans text-[15.5px] leading-[1.6] text-house-brown/70">{lim.note}</p>
              ) : null}
            </div>
          </section>
        );
          ...
          <div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {(data.readiness ?? READINESS_DEFAULT).map((r) => (
              <div key={r.h} className="border border-house-brown/12 bg-house-white p-5">
                <p className="font-display text-[21px] leading-tight text-house-black">{r.h}</p>
                <p className="mt-2 font-sans text-[16.5px] leading-[1.5] text-house-stone">{r.p}</p>
              </div>
            ))}
          </div>
              ...
              {data.relatedCovers.items.filter((c) => !c.href.endsWith(`/${data.slug}`)).slice(0, 3).map((c) => (
                ...
                >
                  ...
                  <div className="flex flex-1 flex-col p-5">
                    <h3 className="font-display text-[22px] leading-tight text-house-black transition-colors group-hover:text-[color:var(--ins-ink)]">{c.name}</h3>
                    {c.body ? <p className="mt-2 font-sans text-[16.5px] leading-[1.55] text-house-stone">{c.body}</p> : null}
                    <span className="mt-4 font-sans text-[14px] tracking-[0.16em] uppercase text-[color:var(--ins-ink)]">View cover →</span>
                  </div>
                </Link>
              ))}
            </div>
      ...
      <section className="px-[5vw] py-14 text-house-cream" style={{ background: "var(--ins-accent)" }}>
        <div className="mx-auto grid max-w-[1120px] items-center gap-10 lg:grid-cols-[1fr_0.66fr] lg:gap-16">
          <div className="max-w-[64ch]">
            <h2 className="font-display text-[clamp(27px,3vw,41px)] leading-[1.12] text-house-cream">
              {data.placed.heading}
            </h2>
            ...
            <div className="mt-7 flex flex-col gap-3 sm:flex-row sm:items-center">
              ...
              <a
                ...
                data-ga-event="cta_click"
                data-ga-cta="call_specialist"
                className="inline-flex items-center justify-center whitespace-nowrap border border-house-cream/40 px-7 py-3.5 font-sans text-[14px] tracking-[0.16em] uppercase text-house-cream no-underline transition-colors hover:border-house-cream"
              >
                Call {SUPPORT_PHONE}
              </a>
            </div>
              ...
              <ul className="mt-6 flex flex-wrap gap-x-6 gap-y-2 p-0">
                {data.crossLinks.map((l) => (
                  <li key={l.href} className="list-none">
                    <Link href={l.href} className="font-sans text-[18px] text-house-gold-light underline underline-offset-2 hover:text-house-cream">
                      {l.label} →
                    </Link>
                  </li>
                ))}
