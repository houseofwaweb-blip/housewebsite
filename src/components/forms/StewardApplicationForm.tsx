...
export function StewardApplicationForm({
  ...
  const selectCls =
    "w-full border border-white/30 bg-transparent px-3 py-2.5 text-[15px] text-white outline-none transition-colors focus:border-[color:var(--color-gold)]";
  const labelCls =
    "block font-sans text-[14px] tracking-[0.04em] mb-1.5 text-white/70";
  ...
  return (
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4" noValidate>
      ...
      {errors.turnstileToken ? (
        <p className="font-sans text-[14px] text-red-300">{errors.turnstileToken.message}</p>
      ) : null}
