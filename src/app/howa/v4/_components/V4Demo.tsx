      ...
      <div className="mx-auto max-w-[1180px] px-6 sm:px-10">
        <div className="grid items-center gap-10 lg:grid-cols-[1fr_auto] lg:gap-16">
          ...
          <div className="min-w-0">
            <p className="smallcaps mb-3 text-[18px] tracking-[0.2em] text-[color:var(--color-gold-deep)]">Step inside</p>
            <h2 className="font-display text-[clamp(30px,2.8vw,45px)] leading-[1.08] tracking-[-0.01em]">
              Try the first <span className="font-italic-display text-[#c5a960]">home record.</span>
            </h2>
            <p className="mt-4 max-w-[460px] text-[18.5px] leading-[1.55] text-[color:var(--color-ink-soft)]">
            ...
            <ul className="mt-7 grid max-w-[480px] grid-cols-1 gap-3 sm:grid-cols-2">
              {can.map((c) => (
                <li key={c.t} className="rounded-xl border border-[color:var(--color-gold)]/20 bg-white p-4">
                  <p className="font-display text-[19px] leading-tight text-[color:var(--color-ink)]">{c.t}</p>
                  <p className="mt-1 text-[18px] leading-snug text-[color:var(--color-ink-soft)]/80">{c.b}</p>
                </li>
              ))}
            </ul>

            <div className="mt-7 flex flex-col items-start gap-3 sm:flex-row sm:items-center">
              <a href="/howa/coming-soon?tier=assistant" className="inline-flex items-center gap-2 rounded-md bg-[color:var(--color-howa-green)] px-6 py-3 text-[19px] text-white transition-colors hover:bg-[color:var(--color-howa-green-deep)]">
                Start with my address <span aria-hidden>→</span>
              </a>
              <a href="#tiers" className="inline-flex items-center gap-1.5 text-[19px] text-[color:var(--color-ink-soft)] hover:text-[color:var(--color-ink)] transition-colors">
