    ...
    <section id="record" className="scroll-mt-20 bg-[#fbfaf5] py-16 lg:py-24 border-t border-[color:var(--color-ink)]/8">
      <div className="mx-auto max-w-[1180px] px-6 sm:px-10">
        <div className="max-w-[720px]">
          <p className="smallcaps mb-3 text-[18px] tracking-[0.2em] text-[color:var(--color-gold-deep)]">One Home Record</p>
          ...
          <p className="mt-4 max-w-[640px] text-[20px] leading-[1.55] text-[color:var(--color-ink-soft)]">
          ...
          <p className="mt-3 smallcaps text-[14px] tracking-[0.16em] text-[color:var(--color-gold-deep)]">Bound to the address, not the phone</p>
        </div>
        ...
        <div className="mt-8 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">
          {writes.map((w) => (
            <div key={w.k} className="rounded-xl border border-[color:var(--color-gold)]/20 bg-white p-4">
              <p className="font-display text-[19px] leading-tight text-[color:var(--color-ink)]">{w.k}</p>
              <p className="mt-1.5 text-[18px] leading-snug text-[color:var(--color-ink-soft)]/80">{w.v}</p>
            </div>
          ))}
        </div>
        <p className="mt-5 text-[18px] text-[color:var(--color-ink-soft)]/70">Everything writes back to this centre.</p>
        <p className="mt-4 font-italic-display text-[clamp(20px,1.7vw,25px)] leading-[1.3] text-[color:var(--color-gold-deep)]">
