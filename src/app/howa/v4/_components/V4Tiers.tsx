    ...
    <section id="tiers" className="scroll-mt-20 bg-[#fbfaf5] py-16 lg:py-24">
      <div className="mx-auto max-w-[1180px] px-6 sm:px-10">
        <div className="mb-10 max-w-[680px]">
          <p className="smallcaps mb-3 text-[18px] tracking-[0.2em] text-[color:var(--color-gold-deep)]">Tiers</p>
            ...
            return (
              ...
              >
                ...
                <div className="flex items-baseline justify-between">
                  <h3 className="font-display text-[27px] leading-none" style={{ color: t.accent }}>{t.name}</h3>
                  <span className={"font-display text-[23px] " + (dark ? "text-[#f3ede0]" : "text-[color:var(--color-ink)]")}>{t.price}</span>
                </div>
                <p className={"mt-2 font-italic-display text-[21px] " + (dark ? "text-[#f3ede0]/85" : "text-[color:var(--color-ink-soft)]")}>{t.tagline}</p>
                <ul className="mt-5 mb-7 space-y-2.5">
                  {t.points.map((p) => (
                    <li key={p} className={"flex items-start gap-2.5 text-[19px] leading-[1.4] " + (dark ? "text-[#f3ede0]/85" : "text-[color:var(--color-ink-soft)]")}>
                ...
                <a
                  ...
                  className={"mt-auto inline-flex items-center justify-center gap-2 rounded-md px-5 py-3 text-[19px] transition-colors " + (dark ? "bg-[#c5a960] text-[#1d2a40] hover:bg-[#d4ba78]" : "text-white")}
        ...
        <p className="mt-6 text-[18px] text-[color:var(--color-ink-soft)]/70">
