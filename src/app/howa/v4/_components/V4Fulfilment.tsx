    ...
    <section id="fulfilment" className="scroll-mt-20 bg-[#fbfaf5] py-16 lg:py-20 border-t border-[color:var(--color-ink)]/8">
      <div className="mx-auto max-w-[1180px] px-6 sm:px-10">
        <div className="grid items-center gap-10 lg:grid-cols-[1.05fr_0.95fr] lg:gap-14">
          <div>
            <p className="smallcaps mb-3 text-[18px] tracking-[0.2em] text-[color:var(--color-gold-deep)]">Optional fulfilment</p>
            <h2 className="font-display text-[clamp(29px,2.8vw,43px)] leading-[1.1] tracking-[-0.01em]">
              Trusted hands, <span className="font-italic-display text-[#c5a960]">always optional.</span>
            </h2>
            <p className="mt-4 text-[18.5px] leading-[1.55] text-[color:var(--color-ink-soft)]">
            ...
            <a href="/howa/coming-soon" className="mt-7 inline-flex items-center gap-2 rounded-md border border-[color:var(--color-howa-green)]/40 px-6 py-3 text-[19px] text-[color:var(--color-howa-green)] transition-colors hover:bg-[color:var(--color-howa-green)]/[0.06]">
        ...
        <div className="mt-10 grid grid-cols-1 gap-3 sm:grid-cols-3">
          {flow.map((s, i) => (
            <div key={s.t} className="rounded-xl border border-[color:var(--color-gold)]/20 bg-white p-5">
              <p className="font-display text-[19px] text-[#c5a960]">{String(i + 1).padStart(2, "0")}</p>
              <p className="mt-1 font-display text-[20px] leading-tight text-[color:var(--color-ink)]">{s.t}</p>
              <p className="mt-2 text-[18px] leading-[1.5] text-[color:var(--color-ink-soft)]/80">{s.b}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="mx-auto max-w-[1180px] px-6 sm:px-10">
        <div className="flex flex-col items-start justify-between gap-6 rounded-2xl border border-[color:var(--color-gold)]/30 bg-white p-8 lg:flex-row lg:items-center lg:p-10">
          <div className="max-w-[640px]">
            <p className="smallcaps mb-2 text-[14px] tracking-[0.18em] text-[color:var(--color-gold-deep)]">Managed Stewardship · bespoke · by application</p>
            <h3 className="font-display text-[clamp(25px,2.2vw,35px)] leading-[1.12] tracking-[-0.005em]">
          ...
          <a href="mailto:sales@willowalexander.co.uk?subject=Managed Stewardship enquiry" className="shrink-0 inline-flex items-center gap-2 rounded-md bg-[#1d2a40] px-7 py-3.5 text-[19px] text-[#f3ede0] transition-colors hover:bg-[#16243b]">
