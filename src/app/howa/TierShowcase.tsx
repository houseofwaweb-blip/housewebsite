...
/* Small inline icons used as feature bullets in the tier columns. */
function LeafIcon() {
  return (
...
const TIERS: {
  ...
  label: string;
  tagline: string;
  /** layout determines arrangement of visual vs features */
  ...
  },
  {
    ...
    visual: {
      ...
      alt: "HoWA Housekeeper phone screen — Today's Plan, tasks orchestrated",
    },
    features: [
  ...
  },
  {
    ...
    visual: {
      ...
      alt: "HoWA Steward phone screen — House Health 91% Optimal",
    },
    features: [
    ...
    <section className={s.tierShowcase}>
      ...
      <div className={s.tierShowcaseGrid}>
        {TIERS.map((tier) => (
          <article
            key={tier.slug}
            className={`${s.tierShowcaseCol} ${s[tier.slug]} ${s[`layout-${tier.layout}`]}`}
          >
            ...
            <div className={s.tierShowcaseBody}>
              <div className={`${s.tierShowcaseVisual} ${s[`v-${tier.visual.kind}`]}`}>
                <Image
                  src={tier.visual.src}
                  alt={tier.visual.alt}
                  width={tier.visual.kind === "phone" ? 426 : 1024}
                  height={tier.visual.kind === "phone" ? 900 : 1228}
