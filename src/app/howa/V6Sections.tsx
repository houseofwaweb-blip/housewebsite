  ...
  return (
    <section id="what" className="bg-[#f4f1e9] py-16 lg:py-24 border-t border-[color:var(--color-ink)]/8 scroll-mt-20">
      <div className="max-w-[940px] mx-auto px-6 sm:px-10 text-center">
        <p className="smallcaps text-[18px] tracking-[0.2em] text-[color:var(--color-gold-deep)] mb-5">What HoWA is</p>
        <p className="font-italic-display text-[clamp(21px,1.8vw,27px)] text-[color:var(--color-ink-soft)] mb-6">
          Not a smart-home gadget. Not a folder of receipts. Not a booking app.
        </p>
        <h2 className="font-display text-[clamp(31px,3.4vw,49px)] leading-[1.12] tracking-[-0.01em] text-[color:var(--color-ink)]">
          ...
          <figcaption className="absolute inset-0 pointer-events-none">
            <div className="hidden md:block absolute top-[11%] left-[4.5%] max-w-[190px]">
              <div className="flex items-center gap-2">
                ...
                <span className="font-mono text-[14px] tracking-[0.2em] uppercase text-[#f3ede0]/90">The HoWA layer</span>
              </div>
              <p className="font-mono text-[14px] tracking-[0.02em] text-[#f3ede0]/55 pl-8 mt-1">understands, then acts</p>
            </div>
            <div className="hidden md:block absolute top-[60%] right-[4.5%] max-w-[200px] text-right">
              <div className="flex items-center justify-end gap-2">
                <span className="font-mono text-[14px] tracking-[0.2em] uppercase text-[#f3ede0]/90">Your home</span>
                <span aria-hidden className="w-6 h-px bg-[color:var(--color-gold)]/80" />
              </div>
              <p className="font-mono text-[14px] tracking-[0.02em] text-[#f3ede0]/55 pr-8 mt-1">every room, asset and document</p>
            </div>
            <span className="hidden sm:block absolute bottom-[7%] left-[4.5%] font-mono text-[14px] tracking-[0.22em] uppercase text-[#f3ede0]/40">Fig. 01 &middot; the layer above</span>
          </figcaption>
        </figure>
      </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-px bg-[color:var(--color-gold)]/20 rounded-lg overflow-hidden border border-[color:var(--color-gold)]/20">
          {beneath.map((c) => (
            <div key={c.who} className="bg-[#f4f1e9] px-5 py-6 text-center">
              <p className="font-display text-[20px] mb-1">{c.who}</p>
              <p className="text-[18px] text-[color:var(--color-ink-soft)]/75">{c.does}</p>
            </div>
          ))}
        </div>
        <p className="text-center mt-8 text-[clamp(19px,1.6vw,23px)] leading-[1.5] text-[color:var(--color-ink-soft)] max-w-[660px] mx-auto">
    ...
    <section id="record" className="bg-[#f4f1e9] py-16 lg:py-20 scroll-mt-20">
      <div className="max-w-[1240px] mx-auto px-6 sm:px-10">
        ...
        <div className="lg:col-span-3">
          <p className={`${hand.className} text-[24px] text-[color:var(--color-gold-deep)] mb-2`}>One continuous record.</p>
          ...
          <p className="mt-4 text-[20px] leading-[1.55] text-[color:var(--color-ink-soft)] max-w-[300px]">
            Everything HoWA learns about your home lives in one place, and when you sell, its memory passes on with it.
          </p>
        </div>
        <div className="lg:col-span-5">
          <div className="relative w-full aspect-[4/3]">
            <Image src="/home-v4/v6-one-record-v2.webp" alt="Three phones in the HoWA Free, HoWA+ and HoWA Steward colourways, with the dollhouse softly behind." fill sizes="(max-width:1024px) 92vw, 45vw" className="object-contain" />
          </div>
        </div>
        <div className="lg:col-span-4 space-y-6">
          {points.map((p) => (
            <div key={p.n} className="flex gap-4">
              <span className="font-display text-[23px] text-[#c5a960] tabular-nums">{p.n}</span>
              <div>
                <p className="font-display text-[22px] leading-[1.2] mb-1">{p.t}</p>
                <p className="text-[19px] leading-[1.5] text-[color:var(--color-ink-soft)] max-w-[260px]">{p.b}</p>
              </div>
            </div>
          ))}
        </div>
...
function RecordDataModel() {
  ...
  return (
    <div className="mt-14 lg:mt-16 border-t border-[color:var(--color-gold)]/20 pt-10">
      <p className="smallcaps text-[14px] tracking-[0.18em] text-[color:var(--color-gold-deep)] mb-5 text-center">
      ...
      <ul className="max-w-[760px] mx-auto space-y-3.5">
        {rows.map((r) => (
          <li key={r.t} className="flex items-start gap-3 text-[19px] leading-[1.5] text-[color:var(--color-ink-soft)]">
...
function StatusTag({ kind }: { kind: "live" | "coming" }) {
  if (kind === "live") {
    return (
      <span className="shrink-0 mt-0.5 smallcaps text-[14px] tracking-[0.14em] rounded-full px-2 py-0.5 bg-[color:var(--color-howa-green)]/12 text-[color:var(--color-howa-green)]">
        Live now
      </span>
    );
  }
  return (
    <span className="shrink-0 mt-0.5 smallcaps text-[14px] tracking-[0.14em] rounded-full px-2 py-0.5 border border-[color:var(--color-ink)]/20 text-[color:var(--color-ink-soft)]/65">
      Coming
    </span>
  );
}
  ...
  return (
    <section id="trust" className="bg-[#f4f1e9] border-y border-[color:var(--color-ink)]/8">
      <div className="max-w-[880px] mx-auto px-6 sm:px-10 py-12 lg:py-14 text-center">
        ...
        <p className="font-display text-[clamp(22px,2vw,29px)] leading-[1.3] text-[color:var(--color-ink)] max-w-[660px] mx-auto">
    ...
    <section id="does" className="bg-[#f4f1e9] pt-16 lg:pt-20 pb-10 scroll-mt-20">
      <div className="max-w-[1240px] mx-auto px-6 sm:px-10">
        <div className="text-center max-w-[680px] mx-auto mb-12">
          <p className="smallcaps text-[18px] tracking-[0.2em] text-[color:var(--color-gold-deep)] mb-3">Live today</p>
          ...
          <p className="mt-4 text-[20px] leading-[1.55] text-[color:var(--color-ink-soft)]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {cards.map((c) => (
            <div key={c.key} className="relative rounded-lg border border-[color:var(--color-gold)]/25 bg-white/45 p-7 flex flex-col">
              ...
              {c.kicker && (
                <p className="smallcaps text-[14px] tracking-[0.16em] text-[color:var(--color-gold-deep)] mt-1.5">{c.kicker}</p>
              )}
              <p className="text-[19px] leading-[1.5] text-[color:var(--color-ink-soft)] mt-3">{c.body}</p>
              {c.examples && (
                <ul className="space-y-1.5 mt-4">
                  {c.examples.map((e) => (
                    <li key={e} className="font-italic-display text-[19px] text-[color:var(--color-ink)]/75 leading-[1.3]">
                      &ldquo;{e}&rdquo;
                    </li>
                  ))}
                </ul>
              )}
              <p className="mt-auto pt-4 border-t border-[color:var(--color-ink)]/8 text-[18px] leading-[1.45] text-[color:var(--color-ink-soft)]/80">
                {c.foot}
              </p>
            </div>
          ))}
  ...
  return (
    <section id="protection" className="relative text-[#f3ede0] overflow-hidden scroll-mt-20" style={{ background: "#27384c" }}>
      <div className="max-w-[1180px] mx-auto px-6 sm:px-10 py-16 lg:py-24 grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-14 items-center">
        ...
        <p className="smallcaps text-[14px] tracking-[0.18em] text-[#f3ede0]/60 mb-5 flex items-center gap-2">
          <span className="w-1.5 h-1.5 rounded-full bg-[#c5a960]" /> Protection
        </p>
        <h2 className="font-display text-[clamp(33px,3.4vw,53px)] leading-[1.08] tracking-[-0.01em] text-[#f3ede0] max-w-[900px]">
          The cost of a home isn&apos;t the mortgage.
        </h2>
        <p className="mt-4 font-display text-[clamp(23px,2vw,33px)] leading-[1.25] text-[#f3ede0]/80 max-w-[780px]">
        ...
        <p className="mt-8 text-[clamp(20px,1.7vw,24px)] leading-[1.5] text-[#f3ede0]/90 max-w-[680px]">
        ...
        <div className="mt-10 flex items-center flex-wrap gap-x-2.5 gap-y-3">
          {flow.map((s, i) => (
            <span key={s} className="flex items-center gap-2.5">
              <span className={"rounded-md border px-3.5 py-2 font-mono text-[18px] tracking-[0.03em] " + (i === flow.length - 1 ? "border-[#c5a960]/55 text-[#c5a960]" : "border-[#f3ede0]/25 text-[#f3ede0]/90")}>
                {s}
              </span>
              {i < flow.length - 1 && (
                <span aria-hidden className="text-[#c5a960] text-[19px]">→</span>
              )}
            </span>
          ))}
          <span className="ml-1 smallcaps text-[14px] tracking-[0.14em] rounded-full px-2 py-0.5 border border-[#f3ede0]/25 text-[#f3ede0]/55">
            Coming
          </span>
        </div>

        <p className="mt-8 text-[18px] leading-[1.5] text-[#f3ede0]/45 max-w-[560px]">
  ...
  return (
    <section className="bg-[#f4f1e9] py-14 border-t border-[color:var(--color-ink)]/8">
      <div className="max-w-[1240px] mx-auto px-6 sm:px-10 grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
        <div className="lg:col-span-5">
          <p className="font-italic-display text-[#c5a960] text-[44px] leading-none mb-2">&ldquo;</p>
          <p className="font-italic-display text-[clamp(26px,2vw,35px)] leading-[1.25] text-[color:var(--color-ink)]">
            HoWA brings calm to the complexity of owning a home.
          </p>
        </div>
        <div className="lg:col-span-4">
          <p className="text-[18.5px] leading-[1.55] text-[color:var(--color-ink-soft)]">
            For the first time we have one place for everything. It saves time, money and so much mental space.
          </p>
          <p className="mt-3 text-[19px] text-[color:var(--color-ink-soft)]/70">Charlotte M., Homeowner</p>
        </div>
        <div className="lg:col-span-3">
    ...
    <section className="bg-[#f4f1e9] py-12 border-t border-[color:var(--color-ink)]/8">
      <div className="max-w-[1320px] mx-auto px-6 sm:px-10 flex flex-col lg:flex-row items-center gap-8">
        <div className="shrink-0 max-w-[230px]">
          <p className="font-display text-[24px] leading-[1.15]">A typical home.</p>
          <p className="text-[19px] text-[color:var(--color-ink-soft)]/70 mt-1">Illustrative of what HoWA holds for a home like yours.</p>
        </div>
        <div className="flex-1 grid grid-cols-2 sm:grid-cols-4 gap-6">
          {stats.map((s) => (
            <div key={s.l}>
              <p className="font-display text-[28px] leading-none">{s.n}</p>
              <p className="text-[18px] text-[color:var(--color-ink-soft)]/70 mt-1.5 leading-[1.3]">{s.l}</p>
            </div>
          ))}
        </div>
  ...
  return (
    <section className="text-[#f3ede0]" style={{ background: "#5f6a49" }}>
      <div className="max-w-[1320px] mx-auto px-6 sm:px-10 py-12 flex flex-col lg:flex-row items-center justify-between gap-6">
        <div>
          <h2 className="font-display text-[clamp(28px,2.4vw,39px)] leading-[1.1]">
            Start with your address.
            <br className="hidden sm:block" /> Meet the home that remembers.
          </h2>
        </div>
        <div className="flex items-center gap-5">
          <p className="text-[19px] text-[#f3ede0]/85 max-w-[220px] hidden sm:block">
            Join the waitlist and be among the first to bring HoWA to your home.
          </p>
          <a href="/howa/coming-soon" className="inline-flex items-center gap-2 rounded-md bg-[#f3ede0] px-7 py-3.5 text-[19px] text-[#27384c] hover:bg-white transition-colors whitespace-nowrap">
            Join the waitlist
          </a>
          <p className={`${hand.className} hidden lg:block text-[23px] text-[#f3ede0]/80`}>We&apos;d love to have you.</p>
        </div>
      </div>
    </section>
  );
