    ...
    <div className="bg-house-cream text-house-brown">
      ...
      <section className="px-[5vw] pb-24">
        <div className="mx-auto grid max-w-[1200px] gap-6 md:grid-cols-2">
          {SERVICE_BUSINESSES.map((b, i) => {
            // A lone final card (odd count) spans the row and centres at
            // half-width so it lines up under the pair above it.
            const alone =
              SERVICE_BUSINESSES.length % 2 === 1 && i === SERVICE_BUSINESSES.length - 1;
            // Services with their own site get a detail page; HoWA-only services
            // link straight out to book.
            const hasSite = Boolean(b.businessUrl);
            const img = (
              <Image
                src={b.image}
                alt={b.name}
            ...
            const mediaClass = "relative block aspect-[16/9] w-full overflow-hidden";
            return (
            <article
              key={b.slug}
              className={`flex flex-col overflow-hidden border border-house-brown/12 bg-house-white${
                alone ? " md:col-span-2 md:mx-auto md:w-[calc(50%-12px)]" : ""
              }`}
            >
              {hasSite ? (
                <Link href={`/services/${b.slug}`} className={mediaClass}>{img}</Link>
              ) : (
                <a href={b.bookUrl} target="_blank" rel="noopener noreferrer" className={mediaClass}>{img}</a>
              )}
              <div className="flex flex-1 flex-col p-7">
                ...
                <h2 className="mt-2 font-display text-[26px] leading-tight text-house-black">
                  {hasSite ? (
                    ...
                    <a href={b.bookUrl} target="_blank" rel="noopener noreferrer" className="no-underline hover:text-house-gold-ink">
                      {b.name}
                    </a>
                  )}
                </h2>
