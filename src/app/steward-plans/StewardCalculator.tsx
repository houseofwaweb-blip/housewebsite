  ...
  return (
    <div className="max-w-[1080px] mx-auto grid grid-cols-1 md:grid-cols-2 gap-[24px] md:gap-[56px] items-start">
      ...
      <div className="bg-house-cream border border-house-brown/12 px-[32px] py-[36px]">
        <h3 className="font-display font-medium text-[25px] mb-[20px]">Your services</h3>
        {SERVICE_OPTIONS.map((s) => (
          <div key={s.index} className="flex justify-between items-center py-[14px] border-b border-house-brown/10 last:border-b-0">
            <label className="font-sans text-[19px] flex items-center gap-[12px] cursor-pointer">
              <input
                type="checkbox"
            ...
            <span className="font-sans text-[18px] text-house-stone">{s.freq}</span>
          </div>
        ))}
      </div>
      ...
      <div className="bg-howa-navy text-house-cream px-[32px] py-[36px] md:sticky md:top-[70px]">
        <div className="font-sans text-[14px] tracking-[0.22em] uppercase text-house-gold-light mb-[8px]">
          Your estimate
        </div>
        <div className="font-display font-medium text-[51px] text-house-cream mb-[4px]">
          £{match.price.toLocaleString()}
        </div>
        <div className="font-sans text-[18px] text-house-cream/60 mb-[24px]">per month</div>

        <div className="font-sans text-[14px] tracking-[0.18em] uppercase text-house-gold-light mb-[6px]">
          Best match
        </div>
        <div className="font-display font-medium text-[27px] mb-[20px]">{match.name}</div>

        <ul className="list-none pl-[16px] text-[18px] leading-[1.8] text-house-cream/78 mb-[24px]">
          {match.list.map((l) => (
        ...
        <div className="flex flex-col gap-[10px]">
          <span className="block w-full text-center px-[32px] py-[14px] font-sans text-[18px] tracking-[0.18em] uppercase text-house-brown bg-house-gold-ink border border-house-gold cursor-default">
            Subscription coming soon
          </span>
          <Link
            href="#open-booking-form"
            className="block w-full text-center px-[32px] py-[14px] font-sans text-[18px] tracking-[0.18em] uppercase text-house-cream border border-house-cream/35 no-underline transition-all duration-[var(--t-base)] ease-out hover:bg-house-cream/10"
          >
